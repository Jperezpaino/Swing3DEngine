...
IDG.insiderReg ={
		...
		submitForm:function(formId,apiUrl){
			...
			$.ajax({ 
				...
				else if(data.status == "regrefresh"){
					IDG.insiderReg.handleRefresh(data);
				}
				return true;
			})
			.fail(function(data){
				...
				$('#insider-popup.modal-box .modal-body .insider-register').prepend('<div class="service-down">Sorry. We are unable to process your request at this time. Please try again later.</div>');
		...
		handleSuccess: function(data){
			...
			IDG.insiderReg.submitToBluekai(requestData);		
		...
		signIn:function(formId,apiUrl){
			...
			$.ajax({ 
				...
				if(data.status == "success")
				 {
					IDG.insiderReg.insiderOmnitureFire();
			...
			.fail(function(data){
				...
				$('#insider-popup.modal-box .modal-body > .insider-signin').prepend('<div class="service-down">Sorry. We are unable to process your request at this time. Please try again later.</div>');
				$('#insider-popup.modal-box .modal-body .insider-signin .loading-icon').css('display','none');
				IDG.insiderReg.log(data);
				return false;
			});
		...
		//read cookie and returns value
			...
			insiderOmnitureFire:function(){
				IDG.insiderReg.log("firing omniture")	
				var s_code=s.t();if(s_code)document.write(s_code);
			},
			
		
			clearOmniturePropEvents:function(){
				IDG.insiderReg.log("Clear Varaibles")	
				s.events="";
			},
			...
			handleRefresh:function(data){
				...
				for(var key in jsonObject){
					var keyValue='';
					if(jsonObject[key] && jsonObject[key] != 'null' && jsonObject[key] == 'email'){
						keyValue=encodeURIComponent(jsonObject[key]);
					}
					else if(jsonObject[key] && jsonObject[key] != 'null' ){
						keyValue=jsonObject[key];
					}
					requestData[key]=keyValue;
				}
				IDG.insiderReg.displayNextStep(requestData);
				
			},
			
			displayAllFields:function(){
				$('.city-field, .country-field').css('display','block');
				$('form .input-row.jobs').css('display','block');
			},
			
			preFillUserData:function(data){
			...
			submitToBluekai:function(data){
				 var map = IDG.insiderReg.varMap()
					for(var key in data){
						var keyValue='';
						if(data[key] && data[key] != 'null' ){
							keyValue=decodeURIComponent(data[key]);
						}
						if(map[key] && map[key] !=null && map[key]!= undefined)
							{
								bk_addPageCtx(map[key],'['.concat(keyValue).concat(']'));
							}
					}	
				 bk_addPageCtx("insider","[true]");
				 bk_ignore_meta = true;
				 bk_doJSTag(17362, 10);
			}
};
