...
Extend the previous modifications and add the functionality to be able to load 
the map information through text files that will contain the distribution of 
coordinates and values, and with these to be able to draw "tile" maps in a 
simple way.

**Video**: _(RyiSnow) - How to Make a 2D Game in Java - Part 4 - Drawing Tiles.mp4_

## [0.5.0] - 03-10-2023 (98,3 %)

The current map system is very limited, therefore, we implemented a world 
loading system, which allows you to load maps larger than the size of the 
screen and at the same time a camera system that will set the player in the 
center of the screen and allow us to scroll to it across the world, displacing 
the world in relation to the player's position.

**Video**: _(RyiSnow) - How to Make a 2D Game in Java - Part 5 - World and Camera.mp4_

## [0.5.1] - 04-10-2023 (98,7 %)

In the previous version we set the controls for the player, and in this one 
version we implemented the controls for the map, both its load and what way 
should be drawn.

**Video**: _(RyiSnow) - How to Make a 2D Game in Java - Part 5 - World and Camera.mp4_

## [0.5.2] - 05-10-2023 (98,7 %)

We have a functional version that is not optimal at all as it draws all the 
whether or not they are inside the screen, we optimize the way they are draw 
the map by not painting the tiles that are outside the camera and only focusing 
on the one that is inside the camera.

**Video**: _(RyiSnow) - How to Make a 2D Game in Java - Part 5 - World and Camera.mp4_

## [0.5.3] - 06-10-2023 (98,7 %)

Continuing with the previous development, we can see that when we adjust to the 
exact size of the screen we get a black border, no 'tiles' drawn, this is due 
to the displacement itself, since in the end it gives rise to 'tiles' that are 
drawn between the edges, that is, its coordinates of position are outside the 
drawing area, but parts of the tile are within this area, therefore, it is 
necessary to extend this area so that this type of situation enter the drawing 
operations and are not excluded.
...
**Video**: _(RyiSnow) - How to Make a 2D Game in Java - Part 6 - Collision Detection.mp4_
